...
class PedidosController extends BaseController
{
    ...
    public function indexAction(){
        ...
        $this->assets->addJs("icontrole/pedidos.bundle.js");
    ...
    public function editAction(){
        ...
        $item = ($this->request->getJsonRawBody())->item;

        /**
         * @var Pedidos $pedido
         */
        $pedido = Pedidos::findById($item->_id->{'$oid'});

        if($item->status->loja !== $pedido->getStatus()->loja){
            $pedido->setStatus($item->status);
        }

        $pedido->setRastreamento($item->rastreamento);

        if($pedido->save()){

            $this->notifyUser($pedido->getId(),$item->status->loja,$pedido->getReferencia());
    ...
    public function notifyUser($idpedido,$status,$referencia){
        ...
        $email->Mensagem($this->template($idpedido,$status,$referencia),true);
