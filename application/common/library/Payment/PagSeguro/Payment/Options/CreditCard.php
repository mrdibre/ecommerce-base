...
class CreditCard implements Payment {
     ...
     * Ex.:
     * {
     *      "token"        => 2ed34e61b24d4ea8ae872c66a512525c,
     *      "installments" =>
     *      {
     *          "total"    => 10,
     *          "valor"    => 500.00
     ...
     *
     */
    public function __construct(array $items, string $hash, \stdClass $comprador, \stdClass $card, \stdClass $urls, \stdClass $frete , int $extra = 0){

        $this->instance = new \PagSeguro\Domains\Requests\DirectPayment\CreditCard();
    ...
    public function Frete(\stdClass $frete){
        ...
        $this->instance->setExtraAmount($frete->valor);
    }

    public function Cartao(\stdClass $card){
        $this->instance->setToken($card->token);
        $this->instance->setInstallment()->withParameters(
            $card->installments->total,
            number_format($card->installments->valor,2,'.',''),
            (int)$card->installments->max_juros,
            (int)$card->installments->max
        );
    }
    ...
    public function Comprador(Sender $comprador){
        ...
        $this->instance->setBilling()->setAddress()->withParameters(
            $comprador->endereco->rua,
            $comprador->endereco->numero,
            $comprador->endereco->bairro,
            $comprador->endereco->cep,
        ...
        $this->instance->setHolder()->setBirthdate($comprador->nascimento);
        $this->instance->setHolder()->setName($comprador->nome);
        $this->instance->setHolder()->setPhone()->withParameters($comprador->contato->ddd, $comprador->contato->telefone);
        $this->instance->setHolder()->setDocument()->withParameters($comprador->documento->tipo,$comprador->documento->valor);
    }
